...
export default class Test implements Command {
    ...
    execute: Execute = async (message:Message, args, connection, user, corp) => {
		...
		if(args[0][0].toLowerCase() === 'inventory') {
			message.channel.send('Inventory Board').then(async sentMessage =>  {
				const messages: string[] = [];
				messages[0] = sentMessage.id;
				// messages[1] = (await message.channel.send('reserved for future use')).id;
				// messages[2] = (await message.channel.send('reserved for future use')).id;
				// messages[3] = (await message.channel.send('reserved for future use')).id;
				if(corp.boards === null) {
					corp.boards = {};
				}
				corp.boards.inventory = messages;

				try {
					await connection.getRepository(Corp).save(corp);
				}
				catch(e) {
					console.log(e);
				}
				...
				inv.queryInvWithPrice({}, corp)
					...
					.then((result) => {
						...
						splitMessage.forEach((m, index) => {
							message.channel.messages.resolve(corp.boards.inventory[index]).edit(m)
						});
