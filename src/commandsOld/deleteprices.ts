...
module.exports = {
	name: 'deleteprices',
	description: 'Use this Command to delete price listings',
	args: false,
	aliases: ['deletep', 'deletep', 'dp'],
	usage: '<planet> and/or <mat>, blank to delete all of your price listings',
	execute(message, args, con) {
		...
		if(queryValues.material && queryValues.planet) {
			const selectInv = new Promise((resolve, reject) => {
				...
				con.query(
					`SELECT * FROM corp_price WHERE mat = '${queryValues.material}' AND planet = '${queryValues.planet}' AND userid = ${userID}`,
					function(err, result) {
						console.log(result);
						if (err) {
							reject(err);
						...
						.then(collected => {
							...
							if (reaction.emoji.name === '✅') {
								...
								const deleteInv = new Promise((resolve, reject) => {
									const messageContents: string[] = [];
									con.query(
										`DELETE FROM corp_price WHERE mat = '${queryValues.material}' AND planet = '${queryValues.planet}' AND userid = ${userID}`,
										function(err, result) {
											if (err) {
												reject(err);
											}
			...
			const selectInv = new Promise((resolve, reject) => {
				const messageContents: string[] = [];
				con.query(
					`SELECT * FROM corp_price WHERE mat = '${queryValues.material}' AND userid = ${userID}`,
					function(err, result) {
						console.log(result);
						if (err) {
							reject(err);
						...
						.then(collected => {
							...
							if (reaction.emoji.name === '✅') {
								...
								const deleteInv = new Promise((resolve, reject) => {
									const messageContents: string[] = [];
									con.query(
										`DELETE FROM corp_price WHERE mat = '${queryValues.material}' AND userid = ${userID}`,
										function(err, result) {
											if (err) {
												reject(err);
											}
			...
			const selectInv = new Promise((resolve, reject) => {
				const messageContents: string[] = [];
				con.query(
					`SELECT * FROM corp_price WHERE planet = '${queryValues.planet}' AND userid = ${userID}`,
					function(err, result) {
						console.log(result);
						if (err) {
							reject(err);
					...
					replyMessage.awaitReactions(filter, { max: 1, time: 60000, errors: ['time'] })
						.then(collected => {
							...
							if (reaction.emoji.name === '✅') {
								...
								const deleteInv = new Promise((resolve, reject) => {
									const messageContents: string[] = [];
									console.log('deleting corp_price');
									con.query(
										`DELETE FROM corp_price WHERE planet = '${queryValues.planet}' AND userid = ${userID}`,
										function(err, result) {
											if (err) {
												reject(err);
											}
			...
			const selectInv = new Promise((resolve, reject) => {
				const messageContents: string[] = [];
				con.query(
					`SELECT * FROM corp_price WHERE userid = ${userID}`,
					function(err, result) {
						console.log(result);
						if (err) {
							reject(err);
						...
						.then(collected => {
							...
							if (reaction.emoji.name === '✅') {
								...
								const deleteInv = new Promise((resolve, reject) => {
									...
									console.log('deleting corp_price');
									con.query(
										`DELETE FROM corp_price WHERE userid = ${userID}`,
										function(err, result) {
											if (err) {
												reject(err);
											}
