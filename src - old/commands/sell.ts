...
module.exports = {
	name: 'sell',
	description: 'Use this command to query what Yellow Fin members want a material that you are trying to sell.',
	args: false,
	aliases: ['b'],
	public: true,
			...
			database.demandQuery(con, queryValues)
				...
				.then((result) => {
						...
						if (databaseResults.length === 0) {
							...
							databaseResults.forEach(function(item, index) {
								...
								if(users.isFIOUser(item.userid)) {
									...
									item.quantity = item.quantity - FIOQuantity;
					...
					if(result.length === 0) {
						...
						message.channel.send(`No YellowFin members appear to need any ${queryValues.material}${planetAppend}.`);
					}
					else {
						const messageContents = [];
						messageContents.push(`The following Yellow Fin members may need ${queryValues.material}${planetAppend}:`);
