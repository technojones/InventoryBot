...
module.exports = {
	name: 'querydemand',
	description: 'Use this Command to query demand',
	args: false,
	aliases: ['queryd', 'qd'],
	...
	execute(message, args, con) {
		...
		if(args.length > 0) {
			...
			database.demandQuery(con, queryValues)
				.then((result) => {
						...
						if (databaseResults.length === 0) {
							...
							databaseResults.forEach(function(item, index) {
								...
								if(users.isFIOUser(item.userid)) {
									...
									if((!queryValues.user && !queryValues.user_id && (groupBy!=='user')) || Array.isArray(queryValues.user)) {
										thisLine += (' needed by ' + item.user);
									}
									else {
										thisLine += (' needed');
									}
									messageContents.push(String(thisLine).padEnd(35, ' ') + '` *' + Math.floor((((Date.now() - FIOTimestamp) / 1000) / 60) / 60) + 'h ago* ▫️');
								}
			...
			const promise = new Promise((resolve, reject) => {
				const messageContents = [];
				con.query(
					`SELECT * FROM demand WHERE userid = '${user.id}' ORDER BY planet, mat`,
					function(err, result) {
						if (err) {
							reject(err);
						}
							...
							messageContents.push(`__Listing Demand Values for ${user.user}__`);
							result.forEach(function(item, index) {
								let planet = functions.planetName(item.planet);
								planet = planet ? planet : item.planet;
								if (lastplanet===item.planet) {
									messageContents.push(String('` ' + item.mat.padEnd(3, ' ') + ': ' + item.quantity).padEnd(30, ' ') + '` *' + Math.floor((((Date.now() - item.timestamp) / 1000) / 60) / 60) + 'h ago* ');
								}
								else {
