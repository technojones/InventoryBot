...
module.exports = {
	name: 'setdemand',
	description: 'Use this Command to set your demand - threshholds that you want your inventory to be over.',
	args: true,
	aliases: ['setd', 'sd'],
		...
		if (args) {
			...
			const promise = new Promise((resolve, reject) => {
				...
				args.forEach(function(item, index) {
					...
					if(!users.isFIOUser(userID)) {
						messageContents.push('**ERROR!** Usage of this command requires that you be setup with FIO');
					...
					if (!queryValues.material) {
						...
						con.query(
							'SELECT * FROM demand WHERE userid = ? AND planet = ? AND mat = ?', [
								userID,
								planet,
								mat,
							], function(err, result) {
								...
								// If there was nothing found, create a new row
									...
									con.query(
										'INSERT INTO demand (userid, planet, mat, quantity, timestamp) VALUES (?, ?, ?, ?, ?)', [
											userID,
											planet,
											mat,
											queryValues.number,
											...
											else messageContents.push('`' + `Added demand for ${queryValues.number} ${mat} on ${outputplanet}` + '`');
								...
								// Otherwise, update the row with the new quantity
									...
									con.query(
										'UPDATE demand SET quantity = ?, timestamp = ? WHERE userid = ? AND planet = ? AND mat = ?', [
											queryValues.number,
											ts,
											userID,
											planet,
											...
											outputplanet = outputplanet ? outputplanet : planet;
											if (queryErr) reject('Database Error: ' + queryErr.sqlMessage);
											else messageContents.push('`' + `Updated demand for ${queryValues.number} ${mat} on ${outputplanet} (prev: ` + result[0].quantity + ')`');
											if (index===(length - 1)) resolve(messageContents);
										});
								}
							});
